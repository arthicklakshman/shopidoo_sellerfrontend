...
const StyledInputLabel = ({ children, required }) => (
  <Box sx={{ display: "flex", alignItems: "center", mb: 1 }}>
    ...
    {required && (
      <Typography
        sx={{ color: "#ef4444", ml: 0.5, fontSize: "14px", fontWeight: 600 }}
      >
        *
      </Typography>
    )}
...
const ImageUploadZone = ({
  ...
  <Box sx={{ mb: 4 }}>
    <Box sx={{ display: "flex", alignItems: "center", mb: 0.5 }}>
      ...
      {required && (
        <Typography
          sx={{ color: "#ef4444", ml: 0.5, fontSize: "15px", fontWeight: 600 }}
        >
          *
        </Typography>
      )}
    ...
    <Box
      ...
      sx={{
        ...
        gap: 1.5,
        "&:hover": {
          backgroundColor: fileData ? "#ffffff" : "#f9fafb",
          borderColor: fileData ? "#d1d5db" : "#9ca3af",
      ...
      {fileData ? (
        ...
        <>
          <FileUploadOutlinedIcon
            sx={{ fontSize: 32, color: error ? "#ef4444" : "#6b7280" }}
          />
          <Typography
            sx={{
              color: error ? "#ef4444" : "#4b5563",
              fontSize: "14px",
              fontWeight: 500,
            }}
          >
...
export default function StoreSetup({ onBack, onNext }) {
  const [formData, setFormData] = useState(() => {
    ...
    return {
      ...
      shippingPreference: parsed.shippingPreference || "platform",
    };
  });
  ...
  useEffect(() => {
    const step2Data = JSON.parse(
      localStorage.getItem("onboarding_step_2") || "{}",
    );
    if (step2Data.addressLine1) {
      ...
      setBusinessAddressText(
        "No business address found. Please go back to Step 2.",
      );
    }
  }, []);
  ...
  const handleFileChange = async (field, file) => {
    ...
    if (file.size > 5000000) {
      setErrors((prev) => ({
        ...prev,
        [field]: "File is too large. Max 5MB.",
      }));
      return;
    }
    try {
  ...
  const handleContinue = async () => {
    ...
    if (!formData.sameAsBusinessAddress) {
      newErrors.pickupAddress = validateRequired(
    ...
    try {
      ...
      if (!sellerId)
        throw new Error("Seller ID missing. Please restart onboarding.");

      const payload = {
        categories: JSON.stringify(formData.selectedCategories),
        shippingPreference: formData.shippingPreference,
        pickupAddress: formData.sameAsBusinessAddress
          ? businessAddressText
          : formData.pickupAddress,
      };

      if (formData.storeLogo?.data) payload.storeLogo = formData.storeLogo.data;
      if (formData.storeBanner?.data)
        payload.storeBanner = formData.storeBanner.data;
        ...
        >
              ...
              <Box sx={{ display: "flex", flexDirection: "column", gap: 2.5 }}>
                <Box sx={{ display: "flex", alignItems: "flex-start" }}>
                  ...
                  <Typography sx={{ color: "#4b5563", fontSize: "14px" }}>
                    ...
                    Use your brand logo for better recognition
                  </Typography>
                </Box>
                <Box sx={{ display: "flex", alignItems: "flex-start" }}>
                  <OrangeBullet />
          ...
          >
            <CardContent sx={{ p: { xs: 3, md: 4 } }}>
              ...
              >
                Let us handle shipping with pre-negotiated rates. Faster
                delivery, better tracking, and hassle-free returns.
              </Typography>
            </CardContent>
          </Card>
        </Grid>
          ...
          <StepWrapper>
            ...
            >
              ...
              >
                <CardContent sx={{ p: { xs: 2, md: 4 } }}> 
                  <ImageUploadZone
                    ...
                    required={false}
                    recommendedSize="Square image recommended (500x500px)"
                    fileData={formData.storeLogo}
                    onChange={(file) => handleFileChange("storeLogo", file)}
                    error={errors.logo}
                    onPreview={handlePreview}
                  />
                  <ImageUploadZone
                    title="Store Banner (Optional)"
                    required={false}
                    recommendedSize="Banner for store page (1200x400px)"
                    fileData={formData.storeBanner}
              ...
              >
                ...
                <Typography
                  sx={{ color: "#ef4444", fontSize: "16px", fontWeight: 600 }}
                >
                  *
                </Typography>
              </Box>
              ...
              <Card
                sx={{
                  borderRadius: "16px",
                  border: "1px solid",
                  borderColor: errors.categories ? "#ef4444" : "#e5e7eb",
                  boxShadow: "none",
                  mb: 4,
                }}
              >
                ...
                <CardContent sx={{ p: { xs: 3, md: 4 } }}>
                  <Box
                    sx={{
                      ...
                      mb: formData.sameAsBusinessAddress ? 2 : 4,
                    }}
                  >
                    <Typography
                      sx={{
                    ...
                    <Switch
                      ...
                      onChange={(e) => {
                        setFormData((prev) => ({
                          ...prev,
                          sameAsBusinessAddress: e.target.checked,
                        }));
                        if (errors.pickupAddress)
                          setErrors((prev) => ({
                            ...prev,
                  ...
                  >
                    ...
                    <Box sx={{ display: "flex", alignItems: "flex-start" }}>
                      ...
                      <Box>
                        ...
                        <Typography sx={{ fontSize: "13px", color: "#6b7280" }}>
                          Use your own courier partners and manage shipping
                          yourself
                        </Typography>
                      </Box>
                    </Box>
